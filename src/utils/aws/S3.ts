import S3, {
  ...
  ListBucketsOutput, ListObjectsOutput, ListObjectsRequest, PutObjectRequest
...
export interface IS3 {
  ...
  uploadFile(bucketName: BucketName, fileName: string, contentType?: ContentType | undefined): ManagedUpload;
}
...
export class S3_Connection implements IS3 {
  ...
  public uploadFile(bucketName: BucketName, fileName: string, contentType?: ContentType | undefined): ManagedUpload {
    const fileStream = fs.createReadStream(fileName).on("error", err => logger.error("FileStream ERROR:", err));
