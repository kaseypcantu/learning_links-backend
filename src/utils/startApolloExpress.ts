...
export const startServer = async () => {
  ...
  app.use(
    cors({
      credentials: true,
      origin: process.env.NODE_ENV === 'development' ? '*' : (process.env.FRONTEND_HOST as string),
    })
  );
  ...
  app.listen(process.env.PORT, () => {
    console.log(
      `Apollo 🚀 Express Server listening on PORT:${process.env.PORT}, visit the GraphQL API @ http://localhost:${process.env.PORT}/graphql 🚀`
    );
  });
