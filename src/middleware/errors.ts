...
export default (err: any, req: Request, res: Response, next: NextFunction) => {
  ...
  if (err instanceof ErrorResponse) {
    ...
    const errorResponse = new ErrorResponse(401, 'Unauthorized', [
      new PrettyError('invalid authentication'),
    ]);
    res.status(errorResponse.statusCode).json(errorResponse);
  } else if (err.status) {
    const errorResponse = new ErrorResponse(err.status, 'unknown', [
      new PrettyError(err.message, err),
    ]);
    res.status(errorResponse.statusCode).json(errorResponse);
  } else {
